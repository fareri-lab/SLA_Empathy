...
if session == 'returning':
   ...
   if version == '1':
       ...
       start = use_version.index(return_trial) #this finds the index of the desired trial spreadsheet in the ver1 or ver2 lists
   elif version == '2':
       use_version = ver2
...
# trial_data = '%s' %(return_version[i])
   ...
   #    event.waitKeys(keyList=('space')) ## insert fmri signal
       ...
       if use_version[i].startswith('Self') and use_version[i].endswith('1.csv') or return_trial == 'Self_SLA_%s.csv' %(block):
           ...
           if not DEBUG and not session == 'returning':
                   instruct_screen.draw()
                   win.flip()
                   event.waitKeys(keyList=('space'))
   ...
   #use_version[i]
       ...
       if use_version[i].startswith('Friend') and use_version[i].endswith('1.csv') or return_trial == 'Friend_SLA_%s.csv' %(block):
       ...
       if use_version[i].startswith('Other') and use_version[i].endswith('1.csv') or return_trial == 'Other_SLA_%s.csv' %(block):
       ...
       for trial in trials:
           ...
           loss_amount = str(abs(float(trial['risky_loss'])))
           dec_place =  abs(Decimal(loss_amount).as_tuple().exponent)
           if dec_place < 2:
               loss_amount = str(format(float(loss_amount), '.2f'))
           else:
               loss_amount = loss_amount

           respcBottom = '%s' %  "-$" +  str(loss_amount)
           ...
           gam_amount = str(abs(float(trial['task_gamble_amount'])))
           dec_place =  abs(Decimal(gam_amount).as_tuple().exponent)
           if dec_place < 2:
               gam_amount = str(format(float(gam_amount), '.2f'))
           else:
               gam_amount = gam_amount
           gamble_amount = '%s' %  "$" +  str(gam_amount)
